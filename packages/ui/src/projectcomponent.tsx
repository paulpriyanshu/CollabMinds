...
import { AppDispatch } from '../../../apps/client-app/app/lib/ProjectsStore';
...
const ProjectComponent = ({ isOpen, title }: ProjectComponentProps) => {
  ...
  const onDropImages = useCallback((acceptedFiles: File[]) => {
    setImageFiles(acceptedFiles.map(file => Object.assign(file as FileWithPreview, {
  ...
  const onDropVideos = useCallback((acceptedFiles: File[]) => {
    setVideoFiles(prevFiles => [
      ...prevFiles,
      ...acceptedFiles.map(file => Object.assign(file as FileWithPreview, {
        preview: URL.createObjectURL(file)
      }))
    ]);
  ...
  const handleUpload = async () => {
    if (videoFiles.length > 0) {
      for (const file of videoFiles) {
        ...
        try {
          ...
          if (file.name) {
            await vidupdate(title, file.name);
          }
    ...
    if (thumbnails) {
      ...
      await thumbnailupdate(title, thumbnails);
    } else {
      setIsModalOpen(prev => !prev);
    }
  ...
  const renderThumbnails = (files: FileWithPreview[]) => files.map(file => (
